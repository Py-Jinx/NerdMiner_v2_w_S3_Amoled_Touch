...
{
  screen_state == 1 ? amoled.lcd_off() : amoled.lcd_on();
  screen_state ^= 1;
}
...
{
  screen_rotation == 0 ? amoled.setRotation(2) : amoled.setRotation(0);
  screen_rotation ^= 0;
}
...
void amoledDisplay_MinerScreen(unsigned long mElapsed)
{
  ...
  amoled.pushColors(0, 0, WIDTH, HEIGHT, (uint16_t *)background.getPointer());
}
